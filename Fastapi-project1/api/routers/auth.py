...
async def register_endpoint(request: Request, data: RegisterPendingUserRequest):
    with engine.begin() as conn:
        return register_pending_user(conn, request.state.current_user["lang"], data)
...
async def refresh_endpoint(request: Request, data: RefreshRequest):
    with engine.begin() as conn:
        return refresh(conn, data.role, request.state.current_user["lang"], data.refresh_token)
